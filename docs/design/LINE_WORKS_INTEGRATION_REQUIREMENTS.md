...
1. 署名検証用の受信層としてGoogle Cloudを利用できるか。
2. Cloud RunとCloud Functionsのどちらを運用しやすいか。未定なら比較して決める。
3. LINE WORKS管理者権限とDeveloper Consoleへアクセスできる担当者は誰か。
4. 最初は1対1トークだけに限定してよいか。
5. 最初の対象機能を地域情報共有にしてよいか。
6. LINE WORKSの利用者IDと既存の営業担当名を誰が対応付けるか。
7. テスト用Bot、テスト用保存先、本番と分離したApps Script環境を用意できるか。
8. Googleフォームとの並行運用期間と、LINE WORKS移行の完了条件をどうするか。
9. LINE WORKSへ送信・表示してよい営業情報の範囲はどこまでか。
